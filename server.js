...
// ✅ Fix CORS to allow all requests from frontend
app.use(cors({
    origin: '*', // Allow any frontend (Change * to your frontend URL for security)
    methods: ['GET', 'POST', 'OPTIONS'],
    allowedHeaders: ['Content-Type']
}));

// ✅ Remove the problematic CSP header or fix it
app.use((req, res, next) => {
    res.setHeader("Content-Security-Policy", "default-src 'self'; connect-src 'self' http://localhost:3000 http://localhost:5500");
    next();
});

const API_KEY = "AIzaSyABCmiB8TTFtfI80yLqxLHnqMWGKBpuXJU"; // Gemini api
